...
def plot_g_psf_model_hist_by_mag(
    ...
    for row, (_, subset_label, subset_mask) in enumerate(masks):
        for col, (bin_label, lo, hi) in enumerate(MAG_BINS):
            ...
            if len(dp1_vals):
                ...
                ax.axvline(np.nanmedian(dp1_vals), color="tab:blue", lw=1.5, ls="-", alpha=0.9)
            if len(dp2_vals):
                ax.hist(dp2_vals, bins=bins, density=True, histtype="step", lw=2.0, color="tab:orange", label=f"DP2 N={len(dp2_vals):,}")
                ax.axvline(np.nanmedian(dp2_vals), color="tab:orange", lw=1.5, ls="-", alpha=0.9)
    ...
    fig.tight_layout()
    return save_fig(fig, plot_dir / f"dp1_dp2_common_g_psf_model_hist_by_mag_{cut_key}_ECDFS.png")
...
def plot_g_mag_diff_hist_by_mag(
    ...
    for row, (_, subset_label, subset_mask) in enumerate(masks):
        for col, (bin_label, lo, hi) in enumerate(MAG_BINS):
            ax = axes[row, col]
            m = subset_mask & mag_bin_selection(common, band, lo, hi) & np.isfinite(values)
            vals = finite_values(values[m])
            if len(vals):
                ...
                ax.text(0.03, 0.96, f"N={len(vals):,}", transform=ax.transAxes, ha="left", va="top", fontsize=8)
            else:
                ax.text(0.5, 0.5, "No objects", transform=ax.transAxes, ha="center", va="center")
...
def main() -> None:
    ...
    for band in bands:
        ...
        if band == "g":
            ...
            for cut_key in ["all_common", "cut_on_DP1", "cut_on_DP2", "unresolved_both"]:
