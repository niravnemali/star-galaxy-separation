...
`data/external/cosmos2020_farmer_truth_catalog.csv` locally for full precision, with GitHub fallback:

`data/external/cosmos2020_farmer_truth_catalog_github.csv`

The full-precision local file is ignored for GitHub because it is larger than 25 MB. The GitHub-safe file keeps only the columns needed for matching and labels.


## Official Plot Display Ranges

Official DP2 CMD/color diagnostic plots are range-limited for readability: magnitudes 15-28 and ordinary colors -1 to 5. This follows the current advisor guidance and prevents a small number of extreme flux-derived outliers from setting the plot axes.
